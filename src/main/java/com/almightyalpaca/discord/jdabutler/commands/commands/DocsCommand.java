...
public class DocsCommand extends ReactionCommand
{
    ...
    public DocsCommand(final Dispatcher.ReactionListenerRegistry registry)
    {
        super(registry);
    }
    ...
    private static Message getDocMessage(final Documentation documentation)
    {
        final EmbedBuilder embed = DocsCommand.getDefaultEmbed().setTitle(documentation.getTitle(), documentation.getUrl());
        ...
        if (documentation.getFields() != null && documentation.getFields().size() > 0)
            for (final Map.Entry<String, List<String>> field : documentation.getFields().entrySet())
            {
                final String fieldValue = String.join("\n", field.getValue());
    ...
    public void dispatch(final User sender, final TextChannel channel, final Message message, final String content, final GuildMessageReceivedEvent event)
    {
        ...
        if (docs.size() == 0)
            ...
            channel.sendMessage(DocsCommand.getDocMessage(docs.get(0))).queue();
        else
        {
            final EmbedBuilder embedB = DocsCommand.getDefaultEmbed().setTitle("Refine your Search");
            for (int i = 0; i < docs.size(); i++)
            {
                final Documentation doc = docs.get(i);
            ...
            final List<String> options = new ArrayList<>(Arrays.asList(Arrays.copyOf(ReactionCommand.NUMBERS, docs.size())));
            ...
            channel.sendMessage(embedB.build()).queue(m -> this.addReactions(m, options, Collections.singleton(sender), 30, TimeUnit.SECONDS, index ->
            {
                ...
                { //cancel button or other error
                    this.stopReactions(m, false);
                    m.delete().queue();
                    return;
                }
                this.stopReactions(m);
                m.editMessage(DocsCommand.getDocMessage(docs.get(index))).queue();
            }));
        }
    }
