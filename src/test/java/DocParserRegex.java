...
public class DocParserRegex {
    ...
    public void testMethodNormal() {
        ...
        assertEquals("Matcher doesn't extract function-name correctly", "funcName", matcher.group(1));
        assertEquals("Matcher doesn't extract function-params correctly", "String a, int b", matcher.group(2));
    }
    ...
    public void testMethodDots() {
        ...
        assertEquals("Matcher doesn't extract function-args with dots correctly", "java.lang.String a", matcher.group(2));
    }
    ...
    public void testMethodGenerics() {
        ...
        assertEquals("Matcher doesn't extract function-args with generics correctly", "Collection<String> a", matcher.group(2));
    }

    @Test
    public void testMethodArrays() {
        Matcher matcher = DocParser.METHOD_PATTERN.matcher("funcName(byte[] a)");
        assertTrue("Matcher can't find methods with arrs", matcher.find());
        assertEquals("Matcher doesn't extract function-args with arrs correctly", "byte[] a", matcher.group(2));
    }
    ...
    public void testMethodCombined() {
        ...
        assertEquals("Matcher doesn't extract function-args with generics and correctly", "java.util.Collection<Permission> permissions, byte[] arr", matcher.group(2));
    }
}
